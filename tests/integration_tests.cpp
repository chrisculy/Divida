...
TEST_CASE("Integration Test", "[integration]")
{
	...
	CHECK(transactions[0].from_person() == grace);
	CHECK(transactions[0].to_person() == arav);
	CHECK(transactions[0].amount() == Approx(77.15f).epsilon(divida::tests::c_moneyEpsilon));

	CHECK(transactions[1].from_person() == marco);
	CHECK(transactions[1].to_person() == arav);
	CHECK(transactions[1].amount() == Approx(39.23f).epsilon(divida::tests::c_moneyEpsilon));
	
	CHECK(transactions[2].from_person() == marco);
	CHECK(transactions[2].to_person() == maggie);
