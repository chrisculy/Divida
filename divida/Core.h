...
#define DECLARE_TO_STRING_SMART_PTRS(type) \
	std::wstring ToString(const std::unique_ptr<type>& ptr); \
	std::wstring ToString(const std::shared_ptr<type>& ptr); \
	std::wstring ToString(const std::weak_ptr<type>& ptr); 

#define IMPLEMENT_TO_STRING_SMART_PTRS(type) \
	std::wstring ToString(const std::unique_ptr<type>& ptr) { return ptr->ToString(); } \
	std::wstring ToString(const std::shared_ptr<type>& ptr) { return ptr->ToString(); } \
	std::wstring ToString(const std::weak_ptr<type>& ptr) \
	{ \
		std::wstring output; \
